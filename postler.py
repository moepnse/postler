...
def run_test(username, maildir, message):
    domain = socket.gethostname()
    print "username:", username, "maildir:", maildir, "domain:", domain
    path = "%s %s%s %s%s" % (os.path.abspath( __file__ ), "--maildir=", maildir, "--username=", username)
...
def main():
    ...
    for o, a in opts:
        if o == "--username":
            ...
            try:
                ...
                log("Error! Unknown User: %s" % username, STD_ERR)
                sys.exit(1)
            uid = userinfo[2]
            gid = userinfo[3]
    ...
    if self_test:
        if test_data_dir is not None:
            read_test_emails(username, maildir, test_data_dir)
